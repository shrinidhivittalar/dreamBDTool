...
export async function fetchRecommendations(payload) {
  ...
  try {
    ...
    // TypeError here means fetch never ran (bad payload, JSON.stringify
    // failure, etc.) - only a genuine network failure should get the canned
    // "backend not connected" message, so callers can tell the two apart.
...
export async function repriceRecommendation(payload) {
  let response
  try {
    response = await fetch(`${API}/api/recommendations/reprice`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload),
    })
  ...
  if (!response.ok) {
    ...
    throw new Error(detail || 'Reprice failed.')
  }
  return response.json()
}
