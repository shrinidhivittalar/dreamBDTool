...
import { categories, MAX_ITEM_COUNT, RUPEE, sweetOptions } from '../config/brief'
...
export function BriefWizard({
  ...
  const itemCountLabel = form.no_item_count_preference ? 'No preference' : `${form.item_count} items`
    ...
    <aside className="card wizard-card h-fit p-5 sm:p-7">
      ...
      {step === 1 ? <div className="wizard-step"><div className="mb-6"><p className="text-sm font-semibold text-[#3a322e]">Let's set the basics</p><p className="mt-1 text-sm text-[#91857d]">Start with the shape and budget of the box.</p></div>
        ...
        <Field label="Number of items" hint="up to 10">
          <div className="stepper"><button type="button" disabled={form.no_item_count_preference} onClick={() => onSet('item_count', Math.max(1, form.item_count - 1))}>-</button><span>{itemCountLabel}</span><button type="button" disabled={form.no_item_count_preference} onClick={() => onSet('item_count', Math.min(MAX_ITEM_COUNT, form.item_count + 1))}>+</button></div>
          <label className="checkbox-row"><input type="checkbox" checked={form.no_item_count_preference} onChange={e => onSet('no_item_count_preference', e.target.checked)} /><span>No preference - let the tool decide</span></label>
          {!form.no_item_count_preference && budgetTooHighForItems && <p className="field-note">Even {form.item_count} of the priciest catalog items ({RUPEE}{Math.round(catalogRange.max)} each) can't reach {RUPEE}{Math.round(form.budget_min).toLocaleString()}. Try raising the item count.</p>}
        </Field>
        ...
        <div className="recap-row"><span className="recap-label">Current brief</span><span className="recap-chip">{RUPEE}{form.budget_min.toLocaleString()}-{RUPEE}{form.budget_max.toLocaleString()}</span><span className="recap-chip">{itemCountLabel}</span><span className="recap-chip">{sweetOptions.find(option => option.value === form.sweet_preference)?.label}</span></div>
