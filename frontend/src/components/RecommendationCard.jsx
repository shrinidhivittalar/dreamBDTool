...
export function RecommendationCard({ recommendation, index, mandatoryProducts, requiredCategories, onToggleCustomization, repricing }) {
  ...
  return (
    <article className="card option-card overflow-hidden" style={{ borderLeft: `4px solid ${ACCENT}` }}>
      <div className="flex items-center justify-between border-b border-[#e6d6ea] px-4 py-2.5">
        <div className="flex items-center gap-2.5">
          <span className="flex h-6 w-6 items-center justify-center rounded-full bg-[#f3e8f6] text-xs font-bold text-[#4c1f53]">
            {String(index + 1).padStart(2, '0')}
          </span>
          <h3 className="text-sm font-bold text-[#301736]">Option {index + 1}</h3>
        </div>
        {index === 0 && (
          ...
          return (
            <div key={product.name} className="border-b border-[#ece0ec] py-1.5 last:border-0">
              <div className="flex items-center justify-between gap-3">
                <span className="flex flex-wrap items-center gap-1.5 pr-3 text-[13px] text-[#4c1f53]">
                  {product.name}
              ...
              {product.customization_eligible && onToggleCustomization && (
                <label className="mt-1 flex items-center gap-1.5 text-[11px] text-[#8a7690]">
                  <input
                    type="checkbox"
      ...
      <div className="bg-[#f8f0f8] px-4 py-2.5">
        <div className="flex items-center justify-between gap-3">
          <div>
            <div className="text-[10px] font-bold uppercase tracking-wider text-[#8a7690]">Recommended price</div>
            <div className="serif mt-0.5 text-lg text-[#301736]">{money(recommendation.dad_selling_price)}</div>
          </div>
          <div className="text-right">
            <div className="text-[10px] font-bold uppercase tracking-wider text-[#8a7690]">Floor price</div>
            <div className="serif mt-0.5 text-lg text-[#a5690a]">{money(recommendation.rock_bottom_price)}</div>
          </div>
        </div>
        {recommendation.packaging_cost > 0 && (
          <div className="mt-1 text-[11px] text-[#8a7690]">+ {money(recommendation.packaging_cost)} packaging</div>
        )}
        {recommendation.customization_surcharge > 0 && (
          <div className="mt-0.5 text-[11px] text-[#8a7690]">+ {money(recommendation.customization_surcharge)} customization</div>
        )}
        <div className="mt-1.5 flex items-center justify-between gap-3 border-t border-[#e6d6ea] pt-1.5">
          <span className="text-[10px] font-bold uppercase tracking-wider text-[#8a7690]">Total</span>
          <span className="text-sm font-bold text-[#301736]">{money(recommendation.total_price)}</span>
        </div>
