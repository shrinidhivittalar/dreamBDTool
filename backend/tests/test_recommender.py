...
def test_mandatory_present_in_every_result():
    request = RecommendationRequest(budget=250, item_count=3, mandatory_products=["Brownie"])
...
def test_ambiguous_mandatory_raises():
    ...
    request = RecommendationRequest(budget=200, item_count=1, mandatory_products=["Deluxe"])
    with pytest.raises(ValueError):
        recommend(catalog, request)


def test_unmatched_mandatory_raises():
    request = RecommendationRequest(budget=200, item_count=1, mandatory_products=["Nonexistent"])
    with pytest.raises(ValueError):
        recommend(_catalog(), request)


def test_mandatory_exceeds_item_count_raises():
    request = RecommendationRequest(budget=1000, item_count=1, mandatory_products=["Brownie", "Cookie"])
    with pytest.raises(ValueError):
        recommend(_catalog(), request)
...
def test_preferred_not_required_for_results():
    ...
    request = RecommendationRequest(budget=250, item_count=3, preferred_products=["Nonexistent Sweet"])
...
def test_over_buffer_and_over_budget_combos_are_ranked_not_dropped():
    request = RecommendationRequest(budget=200, item_count=3, mandatory_products=["Brownie"], buffer_percentage=10)
    recommendations = recommend(_catalog(), request, limit=10)
    totals = [rec.total_price for rec in recommendations]
    scores = [rec.score for rec in recommendations]
    assert len(recommendations) == 10
    assert max(totals) > request.budget  # over-budget combos surfaced, not filtered out
    assert scores == sorted(scores, reverse=True)  # ranked, never a hard cutoff


def test_no_duplicate_products_and_correct_size():
    request = RecommendationRequest(budget=300, item_count=4)
...
def test_no_duplicate_items_with_tied_prices_and_bonuses():
    ...
    request = RecommendationRequest(budget=1000, item_count=6, mandatory_products=["Sandwich 0"], preferred_products=["Sandwich"])
...
def test_item_count_exceeds_pool_returns_empty():
    request = RecommendationRequest(budget=1000, item_count=20)
    assert recommend(_catalog(), request) == []


def test_dp_matches_brute_force_optimum():
    catalog = _catalog()
    request = RecommendationRequest(budget=180, item_count=3, buffer_percentage=15)
    best = recommend(catalog, request, limit=1)[0].score

    target = request.budget * (1 - request.buffer_percentage / 100)
    brute_best = max(
        _closeness(sum(p.selling_price for p in combo), request.budget, target)
...
def test_repeats_dp_matches_brute_force_optimum():
    ...
    request = RecommendationRequest(budget=250, item_count=4, buffer_percentage=15, allow_repeats=True)
...
def test_repeats_result_self_consistent_with_tied_prices():
    ...
    request = RecommendationRequest(budget=1000, item_count=6, preferred_products=["Sandwich"], allow_repeats=True)
    for rec in recommend(catalog, request, limit=20):
        names = [product.name for product in rec.products]
        assert len(names) == request.item_count
        by_name = {product.name: product for product in catalog}
        recomputed_total = sum(by_name[name].selling_price for name in names)
        recomputed_bonus = sum(_bonus_value(by_name[name], request.preferred_products) for name in names)
        target = request.budget * (1 - request.buffer_percentage / 100)
        expected_score = _closeness(recomputed_total, request.budget, target) + recomputed_bonus
...
def test_repeats_score_is_at_least_as_good_as_without():
    request_no_repeats = RecommendationRequest(budget=1000, item_count=4, buffer_percentage=10)
    request_repeats = RecommendationRequest(budget=1000, item_count=4, buffer_percentage=10, allow_repeats=True)
...
def test_required_category_present_in_every_result():
    ...
    request = RecommendationRequest(budget=300, item_count=4, required_categories=["Brownie", "Cookie"])
...
def test_required_category_no_match_raises():
    request = RecommendationRequest(budget=300, item_count=3, required_categories=["Nonexistent Category"])
    with pytest.raises(ValueError):
        recommend(_catalog(), request)
...
def test_required_categories_plus_mandatory_exceed_item_count_raises():
    ...
    request = RecommendationRequest(budget=300, item_count=2, mandatory_products=["Cake"], required_categories=["Brownie", "Cookie"])
    with pytest.raises(ValueError):
        recommend(catalog, request)
...
def test_repeats_and_category_quota_combined_matches_brute_force():
    ...
    request = RecommendationRequest(
        budget=400, item_count=5, buffer_percentage=10, allow_repeats=True,
    ...
    target = request.budget * (1 - request.buffer_percentage / 100)
    brute_best = max(
        _closeness(sum(catalog[i].selling_price for i in combo), request.budget, target)
...
def test_achievable_max_hint_when_catalog_price_ceiling_is_low():
    ...
    request = RecommendationRequest(budget=1000, item_count=2, buffer_percentage=10)
    messages: list[str] = []
    recommendations = recommend(catalog, request, messages=messages)
    assert recommendations
    assert messages
    assert "55" in messages[0] or "priciest" in messages[0].lower()


def test_no_hint_when_budget_is_reachable():
    request = RecommendationRequest(budget=200, item_count=3, buffer_percentage=10)
    messages: list[str] = []
